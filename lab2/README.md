...
세션 서비스와 메모리 서비스는 데이터를 관리하는 목적이 다릅니다. 세션 서비스는 대화 도중에 발생하는 이벤트와 상태 정보를 순차적으로 기록하며 대화의 흐름을 유지합니다. 하지만 이 기록만으로는 대화 내용 중 필요한 정보를 선별하여 찾아내는 기능을 수행하기 어렵습니다. 

반면 메모리 서비스는 대화 내용에서 주요 지식을 추출하여 검색에 최적화된 형태로 저장합니다. 에이전트가 과거 정보를 답변에 활용하려면 이처럼 인덱싱된 데이터를 제공하는 메모리 서비스가 필요합니다.
