...
이번 실습에서 만드는 에이전트는 최신 정보가 필요하면 검색을 하고, 이전 대화 내용이 필요하면 저장된 기억을 불러옵니다. 답변을 마친 뒤에는 현재 대화 내용을 자동으로 저장하는 기능을 콜백 함수를 이용해 구현해봅시다. 잠깐 근데 ADK에서는 다루는 정보는 두가지 방식으로 나뉩니다.
...
>
> 이번 예제에서는 `memory://`로 메모리 동작 방식을 확인합니다. 단, `memory://`는 프로세스가 종료되면 사라진다는 점을 유념하세요. 재시작 후에도 유지되는 장기 기억은 [추가적인 메모리 뱅크 서비스](https://adk.dev/sessions/memory/#choosing-the-right-memory-service)를 연결하여 사용합니다.
...
자 이제 조금 더 자세히 알아볼까요?  아래 명령어로 대화를 시작해보세요.
