...
여기서 많은 분이 혼동하시는 점이 "세션 파일(`session.db`)이 있는데 왜 메모리 서비스가 따로 필요한가?"라는 부분입니다. 두 서비스는 데이터를 다루는 목적이 완전히 다릅니다.

| 구분 | 세션 서비스 (Session Service) | 메모리 서비스 (Memory Service) |
| :--- | :--- | :--- |
| **핵심 목적** | 현재 대화의 **상태 및 이벤트 기록** | 과거 대화에서 추출한 **정보 검색** |
| **데이터 활용** | 대화의 순서, 진행 상태 등을 관리 | 필요한 정보를 찾아 답변에 반영 (RAG) |
| **저장 방식** | 발생한 이벤트를 순차적으로 기록 (로그형) | 검색이 용이하도록 인덱싱하여 저장 (검색형) |

쉽게 말해, 세션 서비스는 대화가 어떻게 흘러갔는지 기록하는 '장부' 역할을 합니다. 하지만 모델이 이 장부를 스스로 뒤져서 과거의 특정 정보를 찾아내기는 어렵습니다. 반면 메모리 서비스는 대화 내용 중 중요한 정보를 따로 뽑아 '검색 엔진'에 넣어두는 것과 같습니다. 따라서 장기 기억을 구현하려면 반드시 메모리 서비스가 필요합니다.

- **session_service_uri**: 현재 대화의 상태를 유지합니다. `sqlite://./outputs/session.db`를 쓰면 재실행 시 이전 대화 흐름을 불러올 수 있지만, 모델이 과거 지식을 검색하는 기능은 제공하지 않습니다.
- **memory_service_uri**: `PreloadMemoryTool` 등이 지식을 검색할 장소를 지정합니다. 로컬 테스트용 `memory://`는 휘발성이므로 프로세스 종료 시 기억이 사라집니다. 영구적인 기억을 위해서는 `agentengine://`과 같은 전용 메모리 뱅크 서비스가 필요합니다.
