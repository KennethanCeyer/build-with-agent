...
`lab2/handson` 폴더로 이동해서 `uv`로 가상환경을 준비합시다. `uv`가 아직 없다면 [uv 설치 문서](https://docs.astral.sh/uv/getting-started/installation/)를 참고하세요. macOS, Linux, WSL 환경에서는 아래 명령어로 설치할 수 있습니다. 설치가 끝난 뒤 새 터미널을 열거나 설치 프로그램이 출력한 안내에 따라 PATH를 반영한 다음 `uv --version`으로 확인합니다.
